...
class Smarty_Compiler extends Smarty {
  ...
  function _compile_file($resource_name, $source_content, &$compiled_content) {
    ...
    $source_content = preg_replace_callback($search, array (
        $this, '_preg_callback'
    ), $source_content);
  ...
  function _compile_tag($template_tag) {
    ...
    if (preg_match('~^\/?' . $this->_reg_obj_regexp . '$~', $tag_command)) {return $this->_compile_registered_object_tag($tag_command, $this->_parse_attrs($tag_args), $tag_modifier);}
    switch ($tag_command) {
      case 'include' :
  ...
  function _compile_insert_tag($tag_args) {
    ...
    if (empty($name)) {return $this->_syntax_error("missing insert name", E_USER_ERROR, __FILE__, __LINE__);}
    if (!preg_match('~^\w+$~', $name)) {return $this->_syntax_error("'insert: 'name' must be an insert function name", E_USER_ERROR, __FILE__, __LINE__);}
  ...
  function _compile_foreach_start($tag_args) {
    $attrs = $this->_parse_attrs($tag_args);
    $arg_list = array ();
    if (empty($attrs['from'])) {return $this->_syntax_error("foreach: missing 'from' attribute", E_USER_ERROR, __FILE__, __LINE__);}
    $from = $attrs['from'];
    if (empty($attrs['item'])) {return $this->_syntax_error("foreach: missing 'item' attribute", E_USER_ERROR, __FILE__, __LINE__);}
    $item = $this->_dequote($attrs['item']);
    if (!preg_match('~^\w+$~', $item)) {return $this->_syntax_error("foreach: 'item' must be a variable name (literal string)", E_USER_ERROR, __FILE__, __LINE__);}
    if (isset($attrs['key'])) {
      $key = $this->_dequote($attrs['key']);
      if (!preg_match('~^\w+$~', $key)) {return $this->_syntax_error("foreach: 'key' must to be a variable name (literal string)", E_USER_ERROR, __FILE__, __LINE__);}
  ...
  function _parse_var_props($val) {
    ...
    elseif (!in_array($val, $this->_permitted_tokens) && !is_numeric($val)) { // literal string
  ...
  function _add_plugin($type, $name, $delayed_loading = null) {
    ...
    if (!isset($this->_plugin_info[$type][$name])) {
      $this->_plugin_info[$type][$name] = array (
          $this->_current_file, $this->_current_line_no, $delayed_loading
      );
    }
  }
      ...
      foreach ($this->_plugins['prefilter'] as $filter_name => $prefilter) {
        if ($prefilter === false) {
          ...
          $_params = array (
              'plugins' => array (
                  array (
                      'prefilter', $filter_name, null, null, false
                  )
              )
          );
      ...
      foreach ($this->_plugins['postfilter'] as $filter_name => $postfilter) {
        if ($postfilter === false) {
          ...
          $_params = array (
              'plugins' => array (
                  array (
                      'postfilter', $filter_name, null, null, false
                  )
              )
          );
          require_once (SMARTY_CORE_DIR . 'core.load_plugins.php');
   ...
   * plugin.
   * return php-code to reflect the transition.
   *
   * @return string
   */
  ...
  function _push_tag($open_tag) {
    array_push($this->_tag_stack, array (
        $open_tag, $this->_current_line_no
    ));
  }
  ...
  function _pop_tag($close_tag) {
    ...
    if (count($this->_tag_stack) > 0) {
      ...
      if ($close_tag == $_open_tag) {return $_open_tag;}
      if ($close_tag == 'if' && ($_open_tag == 'else' || $_open_tag == 'elseif')) {return $this->_pop_tag($close_tag);}
