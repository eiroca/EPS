...
class TPage {
  ...
  function registerChannelLink($exitPoint = false) {
    ...
    if ($url || $exitPoint) {
      ...
      $def['caption'] = $cap;
      $link = new TLink($def);
      $this->register_link('channel', $link);
    }
    else {
      // Parent Servizio
  ...
  function getStyleSheetURL() {
    global $SERVICE;
    return array (
        '' => 'style.php?s=' . $SERVICE->namespace, 'handheld' => 'style.php?s=' . $SERVICE->namespace
    );
  }
...
class TRender {
  ...
  function getTemplateDir() {
    global $CONFIG, $CONTEXT, $SERVICE;
    $subdir = $CONTEXT->renderClass;
    $base1 = SRV_DIR . $SERVICE->namespace . DIR_SEP . $CONFIG['relpath_template'];
    $base2 = '.' . DIR_SEP . $CONFIG['relpath_template'];
    $dirs = array (
        $base2 . DIR_SEP . $SERVICE->namespace . DIR_SEP . $subdir, $base2 . DIR_SEP . $SERVICE->namespace, $base1 . DIR_SEP . $subdir, $base1, $base2 . DIR_SEP . $subdir, $base2
    );
    return $dirs;
  }
  ...
  function getSmarty() {
    ...
    $smarty->plugins_dir = array (
        'plugins', $path, EPS_CORE_DIR . 'plugins'
    );
