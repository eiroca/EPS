...
class Page extends AFOPage {
  ...
  function setup(&$cached) {
    if (!$cached) {
      ...
      $link = new TLink(sprintf("vai a %s di %s", $iid, $count), sprintf("#service#?s=AFO&amp;a=SHOW&amp;t=%s&amp;c=%s&amp;i=%s", $this->kind_id, $this->cid, $iid));
      $this->register_link('AFO_next', $link);
      $link = new TLink($this->afo['main']['label1'], sprintf("#service#?s=AFO&amp;a=SHOW&amp;t=%s&amp;c=%s", $this->kind_id, $this->cid));
      $this->register_link('AFO_random', $link);
      $link = new TLink('vai a...', sprintf('#service#?s=AFO&amp;a=GOTO&amp;t=%s&amp;c=%s', $this->kind_id, $this->cid), 'system_dot3');
      $this->register_link('AFO_elenco', $link);
    }
  }
